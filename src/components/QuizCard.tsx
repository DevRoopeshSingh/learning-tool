...
const QuizCard: React.FC<QuizCardProps> = ({
  ...
  // Memoize the answer function to avoid unnecessary re-renders
  const memoizedHandleAnswer = useCallback(
    (option: string) => handleAnswer(quiz, option),
    [quiz, handleAnswer]
  );
      ...
      <Box
        sx={{
          ...
          backgroundColor: completed.has(quiz.question)
            ? feedback?.includes("Correct")
              ? "rgba(102, 187, 106, 0.15)" // Green for correct
              : "rgba(239, 83, 80, 0.15)" // Red for incorrect
            : "background.paper",
          border: `2px solid ${
        ...
        <Box
          ...
          {quiz.options.map((option) => (
            <Button
              key={option}
              onClick={() => memoizedHandleAnswer(option)}
...
const areEqual = (prevProps: QuizCardProps, nextProps: QuizCardProps) => {
  return (
